    ...
    <main className="min-h-screen bg-paper px-4 py-6 text-ink sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        {activeTab === "Data sources" ? <section className="mt-5 grid gap-4 lg:grid-cols-2">
          <Card title="Market symbols" description="WestWall pulls the latest Nasdaq price and daily change for enabled symbols in priority order.">
          ...
          <Card title="Aircraft location" description="Live ADS-B positions are sorted by distance from this point; stale aircraft are removed automatically."><div className="space-y-2">{data.locations.map((location) => <div key={location.id} className="flex items-center justify-between rounded-xl bg-mist/60 px-4 py-3"><span><strong>{location.name}</strong><small className="ml-2 text-ink/45">{location.radiusMiles} mi · every {location.refreshIntervalSeconds}s</small></span><button onClick={() => void remove("locations", location.id, location.name)} className="text-ink/30 hover:text-clay"><Trash2 size={15} /></button></div>)}</div></Card>
        </section> : null}
